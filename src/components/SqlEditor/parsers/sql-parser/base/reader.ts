...
export async function getCursorInfo(rootStatement: IStatements, keyPath: string[]) {
  ...
  const cursorKey = [...keyPath].pop();
  const parentStatement = get(rootStatement, keyPath.slice(0, -1));
...
export function findNearestStatement(
  ...
  if (keyPath.length > 1) {
    return findNearestStatement(rootStatement, keyPath.slice(0, -1), callback);
  }
  return null;
}
